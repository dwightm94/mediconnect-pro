...
import {
  ...
  Activity, Pill, FileText, Syringe, Heart, ExternalLink, Info
} from 'lucide-react'

// ─── Epic Production Config ──────────────────────────────────────────────────
const EPIC_PROD_CLIENT_ID = '2acacaff-969e-4271-ac34-475e52b8c068' // SANDBOX
const EPIC_SCOPES = 'patient/Patient.read patient/Observation.read patient/Condition.read patient/AllergyIntolerance.read patient/MedicationRequest.read patient/Immunization.read patient/Encounter.read launch/patient openid fhirUser'
...
export default function HealthSourcesPage() {
  ...
  // ─── Direct Epic Connect (Epic shows hospital picker) ─────────────────────
  const handleDirectEpicConnect = useCallback(async () => {
    if (!user?.sub) return
  ...
  const handleOtherEhrConnect = (ehrId: string) => {
    ...
    if (ehrId === 'athenahealth') {
      ...
      const params = new URLSearchParams({
        ...
        redirect_uri: redirectUri, scope: 'patient/Patient.read patient/Observation.read patient/MedicationRequest.read patient/Condition.read patient/AllergyIntolerance.read patient/Immunization.read patient/Encounter.read openid fhirUser',
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-white/90">
              <div className="flex items-start gap-2"><span className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-xs font-bold flex-shrink-0">1</span><span>Click connect and <strong>find your hospital</strong> in Epic&apos;s portal finder</span></div>
              <div className="flex items-start gap-2"><span className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center text-xs font-bold flex-shrink-0">2</span><span>Sign in with your MyChart credentials and approve <strong>read-only access</strong></span></div>
